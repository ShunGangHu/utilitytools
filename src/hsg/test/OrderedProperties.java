...
public class OrderedProperties {
    ...
    private final LinkedHashMap<String, String> keyCommentMapWithKey = new LinkedHashMap<String, String>();
    private final LinkedHashMap<String, String> keyCommentMapAll = new LinkedHashMap<String, String>();
    ...
    public synchronized String setProperty(String key, String value, String comment) {
        ...
        if (BLANK.equals(comment)) {
            if (!keyCommentMap.containsKey(key)&&null!=comment) {
    ...
    private void load0(LineReader lr) throws IOException {
        ...
        while ((limit = lr.readLine()) >= 0) {
            ...
            if (c == '#' || c == '!') {
                ...
                keyCommentMapAll.put(comment, comment);
                continue;
            }
    ...
    private void store0(BufferedWriter bw, boolean escUnicode) throws IOException {
        synchronized (this) {
            ...
            String nokeycomment = saveNoKeyComments();
            if (!nokeycomment.equals(BLANK)){
            	writeCommentsNokey(bw, nokeycomment);
            	//bw.newLine();
            }
        }
        bw.flush();
	...
	private String saveNoKeyComments() {
		Iterator<Map.Entry<String, String>> kvIterall = keyCommentMapAll.entrySet().iterator();
		StringBuffer buffer = new StringBuffer();
		while (kvIterall.hasNext()) {
			boolean flag = false;
			Map.Entry<String, String> entryall = kvIterall.next();
            String commentall = entryall.getValue();
    		Iterator<Map.Entry<String, String>> kvIter = keyCommentMapWithKey.entrySet().iterator();
            while (kvIter.hasNext()) {
            	Map.Entry<String, String> entry = kvIter.next();
                String comment = entry.getValue();
				if (comment != null && null != commentall) {
					if (comment.indexOf(commentall)>-1) {
						flag = true;
						break;
					}
				}
    ...
    private void writeComments(BufferedWriter bw, String comments) throws IOException {
        ...
        while (current < len) {
            ...
            if(c == '\n' || c == '\r'){
            	if (suf.toString().length() > 0) {
					keyCommentMapWithKey.put(suf.toString(),  suf.toString());
				}
    ...
    private void writeCommentsNokey(BufferedWriter bw, String comments) throws IOException {
    	bw.write("#");
    	int len = comments.length();
    	int current = 0;
    	int last = 0;
