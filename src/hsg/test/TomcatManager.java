    ...
    */
    public static void main(String[] args) {
        try {
            ...
            ObjectName threadpoolObjNamep = new ObjectName(
                    "Catalina:type=ThreadPool,*");
            Set<ObjectName> p = mbsc.queryNames(threadpoolObjNamep, null);
            String port = "";
            for (ObjectName obj : p) {
                String porta = obj.getKeyProperty("name");
                if (porta.startsWith("http")) {
                    port = porta;
                }
            }
            // 端口最好是动态取得
            ObjectName threadObjName = new ObjectName(
                    "Catalina:type=ThreadPool,name=" + port);
            ...
            System.out.println("连续工作时间:" + TomcatManager.formatTimeSpan(timespan));
            // ------------------------ JVM -------------------------
            // 堆使用率
